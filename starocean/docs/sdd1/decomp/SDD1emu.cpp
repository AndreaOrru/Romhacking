...
void SDD1_IM::prepareDecomp(const uint8 *in_buf) {
  initial_byte_ptr = in_buf;
  byte_ptr = in_buf;
  bit_count = 4;
}
...
uint8 SDD1_IM::getCodeword(uint8 code_len) {
  ...
  codeword = (*byte_ptr) << bit_count;

  ++bit_count;

  if (codeword & 0x80) {
    codeword |= *(byte_ptr + 1) >> (9 - bit_count);
    bit_count += code_len;
  }
...
uint32 SDD1_IM::getCompSize() { return byte_ptr - initial_byte_ptr; }
...
void SDD1_GCD::getRunCount(uint8 code_num, uint8 *MPScount, bool8 *LPSind) {
  ...
  const uint8 run_count[] = {
      ,
  };
...
void SDD1_CM::prepareDecomp(const uint8 *first_byte) {

  bitplanesInfo = (*first_byte) & 0xc0;
  contextBitsInfo = (*first_byte) & 0x30;
  bit_number = 0;
  for (int i = 0; i < 8; i++)
...
void SDD1_OL::prepareDecomp(const uint8 *first_byte, uint16 out_len,
                            uint8 *out_buf) {
...
uint32 SDD1emu::decompress(const uint8 *in_buf, uint16 out_len,
                           uint8 *out_buf) {
  size_t comp_size;
  ...
  return IM.getCompSize();
}

////////////////////////////////////////////////////////////

SDD1emu::SDD1emu()
    : GCD(&IM), BG0(&GCD, 0), BG1(&GCD, 1), BG2(&GCD, 2), BG3(&GCD, 3),
      BG4(&GCD, 4), BG5(&GCD, 5), BG6(&GCD, 6), BG7(&GCD, 7),
      PEM(&BG0, &BG1, &BG2, &BG3, &BG4, &BG5, &BG6, &BG7), CM(&PEM), OL(&CM) {}
